.PHONY: test-low test-mid test-high test-low-light test-mid-light test-high-light help

# Параметры по умолчанию для нагрузочного тестирования
C ?= 50
D ?= 30
URL_LOW = http://localhost:8083/2_php_fpm/test.php
URL_MID = http://localhost:8084/2_php_fpm/test.php
URL_HIGH = http://localhost:8085/2_php_fpm/test.php
URL_LOW_LIGHT = http://localhost:8083/2_php_fpm/test-light.php
URL_MID_LIGHT = http://localhost:8084/2_php_fpm/test-light.php
URL_HIGH_LIGHT = http://localhost:8085/2_php_fpm/test-light.php

# =========================================
# Помощь
# =========================================
help:
	@echo "======================================="
	@echo "Доступные команды для нагрузочного тестирования:"
	@echo "======================================="
	@echo ""
	@echo "Основные команды:"
	@echo "  make test-all          - Тестирование всех трех сервисов (Low, Mid, High)"
	@echo "  make test-low          - Тестирование Low сервиса (порт 8083)"
	@echo "  make test-mid          - Тестирование Mid сервиса (порт 8084)"
	@echo "  make test-high         - Тестирование High сервиса (порт 8085)"
	@echo ""
	@echo "Легкие тесты (test-light.php):"
	@echo "  make test-low-light    - Тестирование Low сервиса с легким скриптом"
	@echo "  make test-mid-light    - Тестирование Mid сервиса с легким скриптом"
	@echo "  make test-high-light   - Тестирование High сервиса с легким скриптом"
	@echo ""
	@echo "Кастомные параметры:"
	@echo "  make test-mid C=150 D=45"
	@echo "  make test-high C=300 D=120"
	@echo ""
	@echo "Параметры:"
	@echo "  C - количество одновременных соединений (по умолчанию: 100)"
	@echo "  D - длительность теста в секундах (по умолчанию: 30)"
	@echo "======================================="

# =========================================
# Тестирование Low сервиса
# =========================================
test-low:
	@echo "======================================="
	@echo "📊 Тестирование Low сервиса"
	@echo "Порт: 8083"
	@echo "Параметры: $(C) соединений, $(D) секунд"
	@echo "URL: $(URL_LOW)"
	@echo "======================================="
	@npx autocannon -c $(C) -d $(D) $(URL_LOW) --renderStatusCodes

# =========================================
# Тестирование Mid сервиса
# =========================================
test-mid:
	@echo "======================================="
	@echo "📊 Тестирование Mid сервиса"
	@echo "Порт: 8084"
	@echo "Параметры: $(C) соединений, $(D) секунд"
	@echo "URL: $(URL_MID)"
	@echo "======================================="
	@npx autocannon -c $(C) -d $(D) $(URL_MID) --renderStatusCodes

# =========================================
# Тестирование High сервиса
# =========================================
test-high:
	@echo "======================================="
	@echo "📊 Тестирование High сервиса"
	@echo "Порт: 8085"
	@echo "Параметры: $(C) соединений, $(D) секунд"
	@echo "URL: $(URL_HIGH)"
	@echo "======================================="
	@npx autocannon -c $(C) -d $(D) $(URL_HIGH) --renderStatusCodes

# =========================================
# Тестирование с легким скриптом
# =========================================
test-low-light:
	@echo "======================================="
	@echo "📊 Тестирование Low сервиса (легкий скрипт)"
	@echo "Порт: 8083"
	@echo "Параметры: $(C) соединений, $(D) секунд"
	@echo "URL: $(URL_LOW_LIGHT)"
	@echo "======================================="
	@npx autocannon -c $(C) -d $(D) $(URL_LOW_LIGHT) --renderStatusCodes

test-mid-light:
	@echo "======================================="
	@echo "📊 Тестирование Mid сервиса (легкий скрипт)"
	@echo "Порт: 8084"
	@echo "Параметры: $(C) соединений, $(D) секунд"
	@echo "URL: $(URL_MID_LIGHT)"
	@echo "======================================="
	@npx autocannon -c $(C) -d $(D) $(URL_MID_LIGHT) --renderStatusCodes

test-high-light:
	@echo "======================================="
	@echo "📊 Тестирование High сервиса (легкий скрипт)"
	@echo "Порт: 8085"
	@echo "Параметры: $(C) соединений, $(D) секунд"
	@echo "URL: $(URL_HIGH_LIGHT)"
	@echo "======================================="
	@npx autocannon -c $(C) -d $(D) $(URL_HIGH_LIGHT) --renderStatusCodes

