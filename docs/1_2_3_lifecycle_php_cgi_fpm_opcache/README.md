...
sequenceDiagram
    ...
    Note over OS, JITBuffer: Инициализация
    ...
    alt CGI
        ...
        Engine->>JITBuffer: Выделение памяти для JIT кода
        end
    else PHP-FPM
        rect rgba(255, 182, 193, 0.2)
        Note right of Engine: Мастер-процесс уже выполнил<br>инициализацию (1–4)<br>Воркер сразу обрабатывает запрос
        end
    end

    Note over OS, JITBuffer: Обработка запроса
    Engine->>OPC: Есть ли Opcodes для script.php?
    
    alt Cache Miss
        rect rgba(135, 206, 250, 0.2)
        Engine->>OS: openat() & read()
        Engine->>Engine: Лексер → Tokens
        Engine->>Engine: Парсер → AST
        Engine->>Engine: Компиляция → Opcodes
        Engine->>OPC: Сохранение Opcodes в SHM
        end
    else Cache Hit
        rect rgba(255, 182, 193, 0.2)
    ...
    Note over Engine: Исполнение (Interpretation)

    loop Execution Loop
        rect rgba(220, 220, 220, 0.3)
        Engine->>Engine: Интерпретация Opcodes
        Engine->>Engine: Сбор профиля исполнения<br>(hot paths / counters)
        end
    end

    opt Обнаружен Hot Trace > opcache.jit_hot_func
        rect rgba(135, 206, 250, 0.2)
        Engine->>JITCompiler: Передача hot trace для компиляции
        JITCompiler->>JITCompiler: Компиляция в Native Machine Code
        JITCompiler->>JITBuffer: Сохранение Native Code в JIT Buffer
        JITCompiler-->>Engine: Уведомление о готовности
        end
    end

    Note over Engine: Повторное исполнение (JIT Mode)

    loop Execution Loop (после JIT)
        rect rgba(255, 182, 193, 0.2)
        Engine->>JITBuffer: Есть ли Native Code для trace?
        alt JIT Cache Hit
            JITBuffer-->>Engine: Возврат Native Machine Code
            Engine->>OS: Выполнение Native Code на CPU
        else JIT Cache Miss
            JITBuffer-->>Engine: Native Code не найден
            Engine->>Engine: Продолжение интерпретации
        end
        end
    end
    ...
    Engine->>Engine: Очистка памяти запроса<br>деструкторы, временные структуры
    opt CGI Mode
        Engine->>OPC: Инвалидация устаревшего кэша
        Engine->>JITBuffer: Очистка JIT Buffer
    end
```
...
- 📝 Мы видим, что **код компилируется в байт-код при каждом выполнении** — без кэширования PHP проходит полный цикл: чтение файла → лексический анализ → синтаксический анализ → компиляция в Opcodes. Это происходит **каждый раз** при обращении к скрипту.
**OpCache** — это механизм кэширования скомпилированного байт-кода PHP в shared memory (общую память).**С OpCache:** скомпилированный байт-код сохраняется в памяти и переиспользуется между запросами. Этапы чтения и компиляции полностью пропускаются — только выполнение готового кода.
**🚀 Вывод:** OpCache — это не просто "оптимизация", а **обязательный компонент** для production-окружения PHP. Без него производительность падает!
