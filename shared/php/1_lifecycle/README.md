...
Коротко:
```bash
script.php → [php.exe (C-программа - PHP-бинарник) или /usr/bin/php на линуксе ] → 
...
| **2. Чтение конфигов** | Загружает все `php.ini` + все файлы из `conf.d/ (расширения opcache, mysql и тд)` | Полное повторное сканирование при каждом запуске |
